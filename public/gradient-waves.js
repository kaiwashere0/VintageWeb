...
export function initGradientWaves(container, options = {}) {
  ...
  const h = hexToRgb(config.horizonColor);
  const w = hexToRgb(config.waveColor);
  const cr = hexToRgb(config.crestColor);
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uWaveRatio: { value: config.waveRatio },
      uSwell: { value: config.swell },
      uTurbulence: { value: config.turbulence },
      uTilt: { value: config.tilt },
      uZoom: { value: config.zoom },
      uHeight: { value: config.height },
      uFogDepth: { value: config.fogDepth },
      uSteps: { value: detailToSteps(config.detail) },
      uBrightness: { value: config.brightness },
      uOpacity: { value: config.opacity },
      uGrain: { value: config.grain ? 1.0 : 0.0 },
      uGrainIntensity: { value: config.grainIntensity },
      uMouse: { value: new Float32Array([0.5, 0.5]) },
      uParallax: { value: config.parallaxStrength },
